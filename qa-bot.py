...
async def send(message, mention, response):
	if (mention != "")
    ...
    await message.channel.send(mention + response)
...
def removeFromList(element)
	addDataToFile("help", user.id)
	
def addToSupport(user)
...
def canEditList(user)
	return "451240245468332033" in [role.id for role in user.roles] or 
			"339613815849353219" in [role.id for role in user.roles] or 
			"450680633560399872" in [role.id for role in user.roles] or 
			"449385190926712863" in [role.id for role in user.roles]
...
async def listadd(ctx, user: discord.Member):
    if canEditList(ctx.message.author):
        ...
        if not inlist(user.id):
            file = open(dataFiles["list"], "a")
            file.write(user.id + "|" + str(user.display_name.encode('unicode_escape')) + "\r\n")
            file.close()
			message = " has been added to the list."
        else:
            message = " is already on the list."
		await bot.say(user.mention + message)
...
async def list(ctx, limit=10):
    ...
    for line in lines:
        ...
        if len(line) > 1:
            ...
            name = line[index+1:]
            index += 1
            await bot.send_message(guild.get_channel("452213143926734859"),str(index) + ". " + name)
    await bot.send_message(guild.get_channel("452213143926734859"), "Listed " + str(limit) + " out of " + str(total) + ".")

@bot.command(pass_context=True)
async def next(ctx):
    if canEditList(ctx.message.author):
        guild = ctx.message.channel.server
        file = open(dataFiles["list"], "r")
        lines = file.readlines()
        for line in lines:
            index = line.find("|")
            id = line[:index]
...
async def whogo(ctx):
    lines = getLines(dataFiles["list"])
    await bot.say(lines[0] + " is going.")
...
async def on_message(message):
    ...
    if not inhelp(user.id) and len(m) <= 50:
		...
		if response != "":
			await send(message, mention, response)
